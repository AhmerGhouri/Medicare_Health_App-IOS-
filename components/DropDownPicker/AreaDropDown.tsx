...
import { addServiceArea } from '../../app/slices/serviceSlice';
...
export const AreaDropDownPicker: React.FC<data> = props => {
  ...
  const EmptyMessage = (): JSX.Element => {
    return (
      ...
      <View style={styles.emptyMessageContainer}>
        <Text allowFontScaling={false} style={styles.emptyMessageText}>This Service is not Available in any Area</Text>
      </View>
            ...
            <View style={styles.dropDownContainer}>
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    activeOpacity={0.95}
                    style={[item.area_desc === selectedLanguage ? { backgroundColor: 'rgb(147 197 253)' } : null, styles.dropDownList]}
                    onPress={() => {
                      setSelectedLanguage(item.area_desc);
                      dispatch(addServiceArea(item.area_id))
                      setIsClicked(false);
                    }}>
                    <Text allowFontScaling={false} style={styles.dropDownListName}>{item.area_desc}</Text>
                    {
                      item.area_desc === selectedLanguage && (
